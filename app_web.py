...
def draw_thai_text(c, text, x, y, align='left', bold=False):
    ...
    # เพิ่มระยะห่างตัวอักษรเล็กน้อยเพื่อลดปัญหาสระซ้อนใน PDF
    c.setCharSpace(0.3)
...
<style>
    ...
    /* ปรับความสูงของกรอบยอดรวมให้สูงขึ้น (55px) */
    .total-box-container {{
...
def create_pdf():
    ...
    draw_thai_text(c, customer, X_NAME, Y_NAME)
    draw_thai_text(c, date_str, X_DATE, Y_DATE)

    y = START_Y
    for i, r in enumerate(data_rows):
        draw_thai_text(c, str(i+1), X_NO, y, align='center')
        draw_thai_text(c, r['item'], X_ITEM, y)
        if r["qty"]: draw_thai_text(c, str(r["qty"]), X_QTY, y, align='center')
        if r["unit"]: draw_thai_text(c, r["unit"], X_UNIT, y, align='center')
        if r["price"]: draw_thai_text(c, format_number(r["price"]), X_PRICE, y, align='right')
        if r["total"]: draw_thai_text(c, format_number(r["total"]), X_TOTAL, y, align='right')
        y -= 20

    draw_thai_text(c, format_number(total_all), X_SUM, Y_SUM, align='right')
    draw_thai_text(c, thai_baht(total_all), X_SUM_TEXT - 250, Y_SUM_TEXT, align='right')
    if note:
        draw_thai_text(c, note, X_NOTE, Y_NOTE)
...
if st.button("สร้าง PDF", type="primary"):
    pdf_bytes = create_pdf().getvalue()
    clean_name = re.sub(r'[\\/*?:"<>|]', '', customer) if customer else "ทั่วไป"
    st.download_button(
        label="📥 ดาวน์โหลดไฟล์ PDF",
        data=pdf_bytes,
        file_name=f"ใบเสนอราคา_{clean_name}_{date_val.strftime('%d-%m-%Y')}.pdf",
